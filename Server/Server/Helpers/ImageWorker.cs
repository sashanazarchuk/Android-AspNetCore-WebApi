        ...
        public static Bitmap FromBase64StringToImage(this string base64String)
        {
            ...
            try
            {
                using (MemoryStream memoryStream = new MemoryStream(byteBuffer))
                {
                    ...
                    imgReturn = System.Drawing.Image.FromStream(memoryStream);
                    memoryStream.Close();
                    byteBuffer = null;
                ...
                using (Bitmap outBmp = new Bitmap(width, height, PixelFormat.Format24bppRgb))
                {
                    using (Graphics oGraphics = Graphics.FromImage(outBmp))
                    {
                        ...
                        if (watermark)
                        {
                            ...
                            using (System.Drawing.Image watermarkImage = System.Drawing.Image.FromFile(Path.Combine(Directory.GetCurrentDirectory(), "images", "lohika_watermark.png")))
